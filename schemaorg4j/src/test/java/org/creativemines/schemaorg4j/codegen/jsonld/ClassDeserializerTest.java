...
public class ClassDeserializerTest extends DeserializerTest {
    ...
    private String withNoSubclassEntries() {
        return "{\n"
            + "      \"@id\": \"http://schema.org/PaymentCard\",\n"
            + "      \"@type\": \"rdfs:Class\",\n"
            + "      \"http://purl.org/dc/terms/source\": {\n"
    ...
    @Test
    public void testClassCanDeserializeIfThereAreNoSubclasses() throws IOException {
        SchemaClass sc = objectMapper().readValue(withNoSubclassEntries(), SchemaClass.class);
        assertEquals(sc.getSubclassOfIds(), Collections.emptySet());
    }
}
